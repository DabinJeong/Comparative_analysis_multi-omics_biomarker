    ...
    for epoch in range(num_epoch+1):
        ...
        if epoch % test_inverval == 0:
            ...
            if num_class == 2:
                ...
                fpr,tpr,thres_roc = metrics.roc_curve( labels_trte[trte_idx["te"]], te_prob[:,1], pos_label=1)
                precision,recall,thres_pr = metrics.precision_recall_curve( labels_trte[trte_idx["te"]], te_prob[:,1], pos_label=1)
                
                auprc = metrics.auc(recall,precision)
                auroc = metrics.auc(fpr,tpr)
                with open("MOGONET_performance.txt",'w') as f:
                    print("AUROC: {:.5f}".format(auroc),file=f)
                    print("AUPRC: {:.5f}".format(auprc),file=f)
            else:
                print("Test ACC: {:.3f}".format(accuracy_score(labels_trte[trte_idx["te"]], te_prob.argmax(1))))
